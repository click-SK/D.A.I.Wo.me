...
const Profile = () => {
  ...
  useEffect(() => {
    fetch(`${process.env.API_URL}/getall`)
      .then((res) => res.json())
          ...
          <div className="desk_two">
            ...
            <div className="btn_wraper_exchange">
              {inputAmount[fromInitial ? 'USDT' : 'DAIWO'] <= allowances[fromInitial ? 'USDT' : 'DAIWO'] ? (
                <button className="exchange_btn" onClick={() => handleSwap()}>Обміняти</button>
              ) : (
